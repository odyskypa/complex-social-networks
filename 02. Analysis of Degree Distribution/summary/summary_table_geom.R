write_summary <- function(language, file) {
   ...
   q <- as.numeric(sub(".*_(\\d+)", "\\1", language))
...
# Generate Table 1  
source = read.table("summary/list_geometric.txt", 
...
for (x in 1:nrow(source)) {
  art_geom <- rbind(art_geom, write_summary(source$language[x], source$file[x]))
}
colnames(art_geom) <- c("Language", "File", "N", "MaximumDegree", "M/N", "N/M", 
                  "SumDegrees", "SumDegreesLogarithm", "C", "q")
rm(x, source)
return(art_geom)
